...
class MAVRallyLoader(object):
    ...
    def load(self, filename):
        ...
        for line in f:
            ...
            if (a[0].lower() == "rally"):
                self.create_and_append_rally_point(float(a[1]) * 1e7, float(a[2]) * 1e7, float(a[3]) * 100.0, float(a[4]) * 100.0, float(a[5]) * 100.0, int(a[6]))
# Coming Soon:
...
#                self.add_rally_land_from_file(float(a[1]), float(a[2]), float(a[3]))
    ...
    def save(self, filename):
        ...
        for p in self.rally_points:
            f.write("RALLY %f\t%f\t%f\t%f\t%f\t%d\n" % (p.lat * 1e-7, p.lng * 1e-7, p.alt / 100.0, p.break_alt / 100.0, p.land_dir / 100.0, p.flags))
        #Coming soon
